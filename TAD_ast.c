						...
						Tdata cargar_conjunto(){
							...
							void free_ast(Tdata ast) {
								...
								if (ast->nodeType == 1) {
									...
									while (aux != NULL) {
										Tdata next = aux->next;
										// Si el nodo aux es de tipo SET o LIST, se liberar? recursivamente
										free_ast(aux);
										aux = next;
									}
								}
							...
							Tdata carga_lista(){
								...
								void mostrarArbol(Tdata arbol) {
									if(arbol!=NULL){
										...
										case 2:
											printf("{");
											Tdata aux1 = arbol->data;   // ?? ESTO es la lista real
								...
								Tdata prod_cartesiano(Tdata a, Tdata b) {
									...
									while (auxa != NULL) {
										...
										while (auxb != NULL) {
											...
											// A?adimos ese par a la lista de resultados
											append(&prod, par);
